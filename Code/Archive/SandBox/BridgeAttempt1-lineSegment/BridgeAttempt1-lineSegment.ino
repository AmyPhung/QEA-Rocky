...
#define DEBUG false
#define CMD_TIME 5623

float desiredDistanceLeft = 1;
float desiredDistanceRight = 1;
...
extern int32_t distanceRight;
float vLsetpoint, vRsetpoint, pLsetpoint, pRsetpoint;
...
float Kp = 6.78;//6.78
float Ki = 50;
float Jp = 630;//316 //630
float Ji = 800; //  800
float Zp = 0;//0.40; // 0.4
float Yp = 0; // Heading //0.06
...
void loop()
{
  ...
  if (cur_time - prev_time > UPDATE_TIME_MS && start_flag)
  {
    ...
    pLsetpoint = pLarray[cmd_index];
    pRsetpoint = pRarray[cmd_index];

    float setpoint = totalDistanceLeft - pLsetpoint + totalDistanceRight - pRsetpoint;
    if (setpoint > .2) {
      setpoint = .2;
    }
    if (setpoint < -.2) {
      setpoint = -.2;
//    }
    Serial.print("pLsetpoint: "); Serial.println(pLsetpoint);
    Serial.print("setpoint: "); Serial.println(setpoint);

    float E_angle = angle_rad - (angleDesired - (Zp * setpoint / 2));
    ...
    Serial.print("Error: "); Serial.println(Kp * E_angle);
    Serial.print("Error Accum: "); Serial.println(Ki * eangle_rad_accum);

    float vLDesired = Kp * E_angle + Ki * eangle_rad_accum + vLsetpoint; // For turns use - Yp;
    float vRDesired = Kp * E_angle + Ki * eangle_rad_accum + vRsetpoint; // For turns use + Yp;
