...
void loop()
{
  ...
  if (cur_time - prev_time > UPDATE_TIME_MS && start_flag)
  {
    ...
    float E_angle = angle_rad - (angleDesired - pow((Zp * (totalDistanceLeft + totalDistanceRight) / 2),2)); // Throwing in a squared term just because (but actually because its slow to respond when it drifts)
